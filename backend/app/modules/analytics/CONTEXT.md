...
**room_monthly_stats**:
Pre-computed monthly stats per room. Default dashboard view is monthly — filter selector allows changing period. Columns: `room_id`, `year_month`, `total_score`, `max_score`, `percentage`.
...
**Dashboard**:
Web UI that reads ONLY from summary tables for fast performance. Default view is current month, with filter for custom periods.
